...
export default function Selection() {
  ...
  const handleConfirm = async () => {
    ...
    const presentEaters = allEaters.filter((e: Eater) => present.includes(e.id))
    ...
    const presentNames = allEaters.filter((e: Eater) => present.includes(e.id)).map((e: Eater) => e.name)
    const hasCookedNames = allEaters.filter((e: Eater) => hasCooked.includes(e.id)).map((e: Eater) => e.name)
    
    await fetch('/api/history', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ dishwashers: dishwasherNames, present: presentNames, hasCooked: hasCookedNames })
    })
  ...
  const unselectedEaters = eaters.filter(e => !present.includes(e.id) && !hasCooked.includes(e.id))
    ...
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <button onClick={() => router.push('/history')} className="p-2 bg-blue-500 text-white rounded">
            Geschiedenis
          </button>
          <button onClick={() => router.push('/')} className="p-2 bg-gray-500 text-white rounded">
            Terug naar Home
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-4">
        <div className="border-2 border-gray-300 rounded p-4 min-h-96 flex-1">
          ...
          <div className="space-y-2">
            {unselectedEaters.map(eater => (
              <div key={eater.id} className="p-2 bg-gray-100 rounded cursor-pointer">
                <div className="font-medium">{eater.name}</div>
                <div className="flex gap-2 mt-2">
                  <button onClick={() => moveToPresent(eater.id)} className="text-xs p-1 bg-blue-500 text-white rounded">
                    Aanwezig
                  </button>
                  <button onClick={() => moveToHasCooked(eater.id)} className="text-xs p-1 bg-green-500 text-white rounded">
                    Heeft Gekookt
                  </button>
                </div>
              </div>
        ...
        <div className="flex-1 space-y-4">
          <div className="border-2 border-green-300 rounded p-4 min-h-24">
            <h3 className="font-bold mb-4 text-center text-green-600">Heeft Gekookt</h3>
            <div className="space-y-2">
              {hasCookedEaters.map(eater => (
                <div key={eater.id} className="p-2 bg-green-100 rounded cursor-pointer">
                  <div className="font-medium">{eater.name}</div>
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => moveToUnselected(eater.id)} className="text-xs p-1 bg-gray-500 text-white rounded">
                      Verwijderen
                    </button>
                    <button onClick={() => moveToPresent(eater.id)} className="text-xs p-1 bg-blue-500 text-white rounded">
                      Aanwezig
          ...
          <div className="border-2 border-blue-300 rounded p-4 min-h-96">
            <h3 className="font-bold mb-4 text-center text-blue-600">Aanwezig</h3>
            <div className="space-y-2">
              {presentEaters.map(eater => (
                <div key={eater.id} className="p-2 bg-blue-100 rounded cursor-pointer">
                  <div className="font-medium">{eater.name}</div>
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => moveToUnselected(eater.id)} className="text-xs p-1 bg-gray-500 text-white rounded">
                      Verwijderen
      ...
      <div className="text-center mt-8">
        <button onClick={() => setShowDishwasherModal(true)} className="p-3 bg-purple-500 text-white rounded-lg text-lg">
          Afwassers Kiezen
        </button>
      </div>

      <Modal isOpen={showDishwasherModal} onClose={() => setShowDishwasherModal(false)}>
        <h2 className="text-xl font-bold mb-4">Afwassers Kiezen</h2>
        
        <div className="mb-4">
          <h3 className="font-bold text-green-600 mb-2">Heeft Gekookt ({hasCookedEaters.length})</h3>
        ...
        <div className="mb-6">
          <h3 className="font-bold text-blue-600 mb-2">Aanwezig ({presentEaters.length})</h3>
          <div className="space-y-1">
            {presentEaters.map(eater => (
        ...
        <div className="flex gap-2">
          <button onClick={handleConfirm} className="p-2 bg-green-500 text-white rounded flex-1">
            Bevestigen
          </button>
          <button onClick={() => setShowDishwasherModal(false)} className="p-2 bg-gray-300 rounded flex-1">
            Annuleren
          </button>
        </div>
      </Modal>
