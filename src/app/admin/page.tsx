...
export default function Admin() {
  ...
  const resetEaterScore = async (id: number, name: string) => {
    if (!confirm(`Weet je zeker dat je de score van ${name} wilt resetten naar 0?`)) return
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <div className="space-y-2">
          {eaters.map((eater: any) => (
            <div key={eater.id} className="flex justify-between items-center p-2 border rounded">
              <div>
                <span className="font-medium">{eater.name}</span>
                <span className="ml-2 text-sm text-gray-600">Score: {eater.score}</span>
              </div>
      ...
      <Modal isOpen={showResetModal} onClose={() => setShowResetModal(false)}>
        ...
        <p className="mb-4">Weet je zeker dat je alle scores wilt resetten naar 0?</p>
        <div className="flex gap-2">
          <button onClick={resetAllScores} className="p-2 bg-orange-500 text-white rounded flex-1">
            Ja, Reset Alle Scores
          </button>
          <button onClick={() => setShowResetModal(false)} className="p-2 bg-gray-300 rounded flex-1">
            Annuleren
          </button>
        </div>
      </Modal>
